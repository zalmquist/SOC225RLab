...
**Question 1.1: why is there a spike in mid-February, 2018?** Look at the related topics and queries for hints at the answer.


## 2. Comparison

Compare the relative popularity of the following social media platforms using Google Trends: Facebook, Twitter (X), Instagram, and YouTube. 
...
**Question 3.1. Load package `gtrendsR`. Load package `tidyverse`. Load package `ggplot2`**
...
Now we can ignore most of that: Google has removed the capability of gtrends to pull data, but the previous TA's for this course helped us out and created an .rda file that includes the records we want. We'll load that in here, use it to create the graphs used in this section.

```{r}
load("~/COURSE_MATERIALS/Lab/Lab6/gtrends_data.Rda")
```

**Question 3.2. Take a glimpse at `hurricanes`. What does the `gtrends` function actually return?**
...
**Question 3.3. Make and plot ten years' worth of search data from 2008-01-01 to 2018-01-01 in US for "spring break" and "vacation" using the provided data.**
...
**Question 3.5. Use object "guns". Take a glimpse at `guns`.**
```{r}
...
**Question 3.8. Use `gunsMerged`, create a map that shows Google search interest for guns in each state using `ggplot2` and `geom_polygon`. Fill the map with the log of `hits`. Properly name your map. (An example map is provided in the first map, try making your own)**
...
## NOTE ON THIS SECTION: Google recently broke the Trends embed capability. Opening these links may not work for you
## If you're seeing error messages, indicate so in 4.1 for credit.
...
**Question 5.1. Search for another set of terms of interest, and embed the results below.** (See this article for some examples: https://techland.time.com/2014/01/16/google-books-ngram-viewer/)
